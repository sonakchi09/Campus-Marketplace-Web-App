...
export default function AdminPage() {
  ...
  useEffect(() => {
    // Read all products from RTDB products node
  ...
  const updateStatus = async (id: string, status: Status) => {
    await update(ref(db, `products/${id}`), { status });
    if (selected?.id === id) setSelected((s) => s && { ...s, status });
  };

  const approveAll = async () => {
    const pending = products.filter((p) => p.status === "pending");
    await Promise.all(pending.map((p) => update(ref(db, `products/${p.id}`), { status: "approved" })));
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between sticky top-0 z-30 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-lg font-bold text-gray-900 tracking-tight">
              Campus<span className="text-yellow-400">Cart</span>
            </span>
        ...
        {counts.pending > 0 && (
          <button
            onClick={approveAll}
            className="flex items-center gap-2 bg-yellow-400 hover:bg-yellow-500 text-black text-sm font-semibold px-5 py-2 rounded-full transition-all hover:scale-105 shadow-md shadow-yellow-200"
          >
            <CheckCircle size={15} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={stat.label} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center gap-4">
              <div className={`w-11 h-11 rounded-xl ${stat.accent} flex items-center justify-center ${stat.color}`}>
                {stat.icon}
              </div>
              <div>
            ...
            {(["all", "pending", "approved", "rejected"] as const).map((f) => (
              <button
                ...
                className={`px-4 py-2.5 rounded-xl text-sm font-medium capitalize transition-all border ${
                  filter === f
                    ? "bg-yellow-400 text-black border-yellow-400 shadow-md shadow-yellow-100"
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="text-center py-12 text-gray-400">No products match your search</td>
                      </tr>
                    ) : (
                      filtered.map((product) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-5 py-3.5 font-bold text-gray-800">₹{Number(product.price).toLocaleString()}</td>
                            <td className="px-5 py-3.5">
                            ...
                            <td className="px-5 py-3.5">
                              <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                                {product.status !== "approved" && (
                                  <button onClick={() => updateStatus(product.id, "approved")} className="p-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-600 transition" title="Approve">
                                    <CheckCircle size={16} />
                                  </button>
                                )}
                                {product.status !== "rejected" && (
                                  <button onClick={() => updateStatus(product.id, "rejected")} className="p-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 transition" title="Reject">
                                    <XCircle size={16} />
                                  </button>
                                )}
                                ...
                                <button onClick={() => deleteProduct(product.id)} className="p-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 transition" title="Delete">
                                  <XCircle size={16} />
                                </button>
                              </div>
                            </td>
            ...
            {selected && (
              <div className="w-72 bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col gap-4 self-start sticky top-24">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-yellow-500">₹{Number(selected.price).toLocaleString()}</span>
                </div>
                ...
                {selected.status !== "approved" && (
                  <button onClick={() => updateStatus(selected.id, "approved")} className="w-full bg-yellow-400 hover:bg-yellow-500 text-black font-bold py-2.5 rounded-xl transition hover:scale-105 flex items-center justify-center gap-2">
                    <CheckCircle size={16} /> Approve Product
                  </button>
                )}
                {selected.status !== "rejected" && (
                  <button onClick={() => updateStatus(selected.id, "rejected")} className="w-full bg-red-50 hover:bg-red-100 text-red-500 font-bold py-2.5 rounded-xl transition flex items-center justify-center gap-2 border border-red-100">
