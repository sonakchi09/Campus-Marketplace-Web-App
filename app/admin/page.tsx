...
export default function AdminPage() {
  ...
  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem("pendingProducts") || "[]");
    setProducts(stored);
  }, []);

  // Save back to localStorage whenever products change
  ...
  const approveAll = () => {
    const updated = products.map((p) =>
      p.status === "pending" ? { ...p, status: "approved" as Status } : p
    );
    persist(updated);
  };
        ...
        {products.length === 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center">
            ...
            <p className="text-gray-400 text-sm">
              When sellers submit products via the Upload page, they'll appear here for review.
            </p>
          </div>
        )}
                        ...
                        return (
                          <tr
                            ...
                            className={`border-b border-gray-50 hover:bg-yellow-50/40 transition-colors cursor-pointer ${
                              selected?.id === product.id ? "bg-yellow-50" : ""
                            }`}
                          >
              ...
              <div className="w-72 bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col gap-4 self-start sticky top-24">
                ...
                {selected.status !== "approved" && (
                  <button
                    ...
                    className="w-full bg-yellow-400 hover:bg-yellow-500 text-black font-bold py-2.5 rounded-xl transition hover:scale-105 flex items-center justify-center gap-2"
                  >
                ...
                {selected.status !== "rejected" && (
                  <button
                    ...
                    className="w-full bg-red-50 hover:bg-red-100 text-red-500 font-bold py-2.5 rounded-xl transition flex items-center justify-center gap-2 border border-red-100"
                  >
